...
function AnnounceParcelPanel() {
  ...
  const [barcode, setBarcode] = useState(() => `DHL-IN-${String(Math.floor(Math.random() * 900) + 100)}`);
        ...
        <button
          ...
          onClick={() =>
            announce.mutate(
              ...
              {
                onSuccess: () => setBarcode(`DHL-IN-${String(Math.floor(Math.random() * 900) + 100)}`),
              },
            )
          }
        >
